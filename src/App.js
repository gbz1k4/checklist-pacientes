...
const SUPABASE_URL = "https://jdokqxueufplidhosqtk.supabase.co";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Impkb2txeHVldWZwbGlkaG9zcXRrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ3MTI4MjUsImV4cCI6MjA5MDI4ODgyNX0.VjiVV7Nr0xTq-CZifb9-07odGQVdc4bVZSfxEQVbNFo";
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .add-form {
    display: flex; gap: 8px; background: #141414;
    border: 1px solid #2a2a2a; border-radius: 10px;
    padding: 10px 14px; margin-bottom: 12px; align-items: center;
  }
  .add-form input {
    flex: 1; border: none; outline: none; font-size: 13px;
    font-family: 'DM Sans', sans-serif; color: #e8e6e0; background: transparent;
  }
  .add-form input::placeholder { color: #444; }

  .btn-add {
    background: #e8e6e0; color: #0a0a0a; border: none; border-radius: 7px;
  ...
  .toolbar input {
    flex: 1; border: 1px solid #2a2a2a; border-radius: 8px;
    padding: 9px 14px; font-size: 13px; font-family: 'DM Sans', sans-serif;
    background: #141414; color: #e8e6e0; outline: none; transition: border-color .15s;
  }
  ...
  .btn-send {
    border: 1px solid #2a2a2a; background: #141414; border-radius: 8px;
    padding: 9px 16px; font-size: 13px; font-family: 'DM Sans', sans-serif;
    cursor: pointer; color: #e8e6e0; transition: background .15s; white-space: nowrap;
  }
  .btn-send:hover { background: #1e1e1e; }

  .hint { font-size: 11px; color: #444; margin-bottom: 28px; font-family: 'DM Mono', monospace; }

  .section-title {
    font-size: 11px; font-weight: 600; letter-spacing: .08em;
    text-transform: uppercase; color: #444; margin-bottom: 8px; padding-left: 2px;
  }
  ...
  .checkbox {
    width: 16px; height: 16px; border: 1.5px solid #333; border-radius: 4px;
  ...
  .pill-sent  { cursor: pointer; transition: opacity .15s, transform .1s; user-select: none; }
  .pill-sent:hover { opacity: .75; transform: scale(1.05); }

  .tbl-input {
    border: 1px solid #2a2a2a; border-radius: 6px; padding: 5px 8px;
    font-size: 12px; font-family: 'DM Sans', sans-serif;
    background: #0a0a0a; color: #e8e6e0; outline: none; width: 60px; transition: border-color .15s;
  }
  ...
  .tbl-select {
    border: 1px solid #2a2a2a; border-radius: 6px; padding: 5px 8px;
    font-size: 12px; font-family: 'DM Sans', sans-serif;
    background: #0a0a0a; color: #e8e6e0; outline: none; max-width: 150px; transition: border-color .15s;
  }
  .tbl-select:focus { border-color: #555; }

  .btn-loc {
    border: 1px solid #2a2a2a; background: #141414; border-radius: 6px;
    padding: 5px 10px; font-size: 11px; font-family: 'DM Sans', sans-serif;
    cursor: pointer; color: #888; transition: background .15s; white-space: nowrap;
  }
  .btn-loc:hover { background: #1e1e1e; color: #ccc; }

  .btn-del {
    border: none; background: transparent; cursor: pointer; color: #333;
    font-size: 18px; padding: 2px 6px; border-radius: 4px; transition: color .15s; line-height: 1;
  }
  ...
  .toast {
    ...
    background: #e8e6e0; color: #0a0a0a; padding: 9px 20px; border-radius: 10px;
  ...
  .empty-row td { padding: 28px 14px; color: #333; font-size: 13px; text-align: center; }
`;
...
export default function App() {
  ...
  async function addPatient() {
    ...
    const { data, error } = await supabase
      .from("patients")
      .insert([{ name, volumes: 0, sent: false, nurse: "", local: "almoxarifado", done: false }])
      .select()
      .single();
    if (!error && data) {
  ...
  async function updateField(id, field, value) {
    ...
    if (field === "volumes") updates.sent = parseInt(value) > 0;
    const { error } = await supabase.from("patients").update(updates).eq("id", id);
    if (!error) setPatients(prev => prev.map(p => p.id === id ? { ...p, ...updates } : p));
  }

  async function toggleSent(id) {
    const p = patients.find(x => x.id === id);
    const newVal = !p.sent;
    const { error } = await supabase.from("patients").update({ sent: newVal }).eq("id", id);
    if (!error) setPatients(prev => prev.map(x => x.id === id ? { ...x, sent: newVal } : x));
  }

  async function toggleLocal(id) {
    const p = patients.find(x => x.id === id);
    const newVal = p.local === "almoxarifado" ? "farmácia" : "almoxarifado";
    const { error } = await supabase.from("patients").update({ local: newVal }).eq("id", id);
    if (!error) setPatients(prev => prev.map(x => x.id === id ? { ...x, local: newVal } : x));
  }

  async function toggleDone(id) {
    const p = patients.find(x => x.id === id);
    const newVal = !p.done;
    const { error } = await supabase.from("patients").update({ done: newVal }).eq("id", id);
    if (!error) setPatients(prev => prev.map(x => x.id === id ? { ...x, done: newVal } : x));
  }
  ...
  async function handleMessage() {
    ...
    const p = patients.find(x =>
      parsed.patientName && x.name.toLowerCase().includes(parsed.patientName.toLowerCase())
    );
    if (!p) { showToast("Paciente não encontrado."); return; }
    const updates = {};
    const changed = [];
    if (parsed.volumes !== undefined) { updates.volumes = parsed.volumes; updates.sent = parsed.volumes > 0; changed.push(parsed.volumes + " vol"); }
    if (parsed.nurse) { updates.nurse = parsed.nurse; changed.push(parsed.nurse); }
    if (parsed.local) { updates.local = parsed.local; changed.push(parsed.local); }
    const { error } = await supabase.from("patients").update(updates).eq("id", p.id);
    if (!error) {
      setPatients(prev => prev.map(x => x.id === p.id ? { ...x, ...updates } : x));
      setMsg("");
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {loading && <div className="loading">Carregando pacientes...</div>}
...
function PatientTable({ patients, updateField, toggleLocal, toggleSent, toggleDone, removePatient, isDone }) {
  return (
    <div className="table-wrap">
      <table>
        <thead>
            ...
            <tr key={p.id} className={isDone ? "done-row" : ""}>
              ...
              <td>
                <input
                  ...
                  className="tbl-input"
                  onChange={e => updateField(p.id, "volumes", parseInt(e.target.value) || 0)}
                />
              </td>
              <td>
                <span
              ...
              <td>
                <select value={p.nurse} className="tbl-select" onChange={e => updateField(p.id, "nurse", e.target.value)}>
